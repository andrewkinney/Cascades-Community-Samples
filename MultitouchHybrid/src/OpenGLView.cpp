...
void OpenGLView::handleScreenEvent(bps_event_t *event) {
    int screenEvent;
    int buttons;
    int position[2];
    int realPosition[2];
	...
	switch (screenEvent) {
		case SCREEN_EVENT_MTOUCH_TOUCH:
			...
			multitouch(new MultitouchEvent(screenEvent, position[0], position[1], (float)realPosition[0], (float)realPosition[1], touchID, touchPressure, this));
			break;
...
//
// move to Cascades control
/*
void OpenGLView::onTouch(bb::cascades::TouchEvent *event) {
	if (event->isDown()) {
		qDebug() << "on touch down: " << event->localX() << " : " << event->localX();
	}
	if (event->isUp()) {
		qDebug() << "on touch up: " << event->localX() << " : " << event->localX();
	}
	if (event->isMove()) {
		qDebug() << "on touch move: " << event->localX() << " : " << event->localX();
	}
}
*/
